...
Future<void> verifyUserFeedbackTable(Database db) async {
...
Future<String> addUserFeedback({
  ...
  await verifyUserFeedbackTable(db); // Ensure table exists and is correct
  const uuid = Uuid();
  ...
  switchBoard.signalOutboundSyncNeeded(); // Signal that new data is available for sync
  return newId;
}

/// Retrieves all user feedback records that need to be sent to the server.
Future<List<Map<String, dynamic>>> getUnsyncedUserFeedback(Database db) async {
  await verifyUserFeedbackTable(db);
...
Future<void> markUserFeedbackAsSynced(List<String> ids, Database db) async {
  if (ids.isEmpty) {
...
Future<int> deleteLocalUserFeedback(List<String> ids, Database db) async {
  if (ids.isEmpty) {
