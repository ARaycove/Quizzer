...
Future<void> verifyUserStatsEligibleQuestionsTable(Database db) async {
...
Future<void> updateEligibleQuestionsStat(String userId, Database db) async {
  ...
  await verifyUserStatsEligibleQuestionsTable(db);
...
Future<List<Map<String, dynamic>>> getUserStatsEligibleQuestionsRecordsByUser(String userId, Database db) async {
  await verifyUserStatsEligibleQuestionsTable(db);
...
Future<Map<String, dynamic>> getUserStatsEligibleQuestionsRecordByDate(String userId, String recordDate, Database db) async {
  await verifyUserStatsEligibleQuestionsTable(db);
...
Future<Map<String, dynamic>> getTodayUserStatsEligibleQuestionsRecord(String userId, Database db) async {
  final String today = DateTime.now().toUtc().toIso8601String().substring(0, 10);
  return await getUserStatsEligibleQuestionsRecordByDate(userId, today, db);
}

Future<List<Map<String, dynamic>>> getUnsyncedUserStatsEligibleQuestionsRecords(Database db, String userId) async {
...
Future<void> upsertUserStatsEligibleQuestionsFromInboundSync(Map<String, dynamic> record, Database db) async {
