...
Future<void> verifyUserStatsTotalQuestionsAnsweredTable(Database db) async {
...
Future<void> incrementTotalQuestionsAnsweredStat(String userId, Database db, bool isCorrect) async {
  final String today = DateTime.now().toUtc().toIso8601String().substring(0, 10);
  await verifyUserStatsTotalQuestionsAnsweredTable(db);
...
Future<List<Map<String, dynamic>>> getUserStatsTotalQuestionsAnsweredRecordsByUser(String userId, Database db) async {
  await verifyUserStatsTotalQuestionsAnsweredTable(db);
...
Future<Map<String, dynamic>> getUserStatsTotalQuestionsAnsweredRecordByDate(String userId, String recordDate, Database db) async {
  await verifyUserStatsTotalQuestionsAnsweredTable(db);
  ...
  if (results.isEmpty) {
    QuizzerLogger.logMessage('No total questions answered record found for userId: $userId and date: $recordDate.');
...
Future<Map<String, dynamic>> getTodayUserStatsTotalQuestionsAnsweredRecord(String userId, Database db) async {
  final String today = DateTime.now().toUtc().toIso8601String().substring(0, 10);
  return await getUserStatsTotalQuestionsAnsweredRecordByDate(userId, today, db);
}

Future<List<Map<String, dynamic>>> getUnsyncedUserStatsTotalQuestionsAnsweredRecords(Database db, String userId) async {
  ...
  QuizzerLogger.logSuccess('Fetched [38;5;10m${results.length}[0m unsynced total questions answered records for user $userId.');
  return results;
}
...
Future<void> upsertUserStatsTotalQuestionsAnsweredFromInboundSync(Map<String, dynamic> record, Database db) async {
