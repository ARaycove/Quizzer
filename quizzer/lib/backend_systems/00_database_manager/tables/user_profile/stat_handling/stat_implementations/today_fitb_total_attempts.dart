...
class TodayFitbTotalAttempts extends StatField{
  static final TodayFitbTotalAttempts _instance = TodayFitbTotalAttempts._internal();
  factory TodayFitbTotalAttempts() => _instance;
  TodayFitbTotalAttempts._internal();

  @override
  String get name => "today_fitb_total_attempts";
  ...
  @override
  String get description => "Total number of Fill-in-the-blank attempts made today.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      ...
      // If we have a new answer for a FITB question, increment
      if (isCorrect != null && questionType == 'fill_in_the_blank') {
        _cachedValue += 1;
      }
      
      return _cachedValue;
    } catch (e) {
      QuizzerLogger.logError('Failed to recalculate today_fitb_total_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for today_fitb_total_attempts: $e');
      rethrow;
    }
  }
}