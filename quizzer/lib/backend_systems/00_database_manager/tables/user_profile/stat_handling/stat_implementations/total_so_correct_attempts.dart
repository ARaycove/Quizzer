...
class TotalSoCorrectAttempts extends StatField{
  static final TotalSoCorrectAttempts _instance = TotalSoCorrectAttempts._internal();
  factory TotalSoCorrectAttempts() => _instance;
  TotalSoCorrectAttempts._internal();

  @override
  String get name => "total_so_correct_attempts";
  ...
  @override
  String get description => "Total number of correct Sort Order attempts made across all time.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      // If we have a new correct Sort Order answer, increment the total
      ...
      QuizzerLogger.logError('Failed to recalculate total_so_correct_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for total_so_correct_attempts: $e');
      rethrow;
    }
  }
}