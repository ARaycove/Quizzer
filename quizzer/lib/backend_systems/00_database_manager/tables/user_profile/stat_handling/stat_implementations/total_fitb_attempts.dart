...
class TotalFitbAttempts extends StatField{
  static final TotalFitbAttempts _instance = TotalFitbAttempts._internal();
  factory TotalFitbAttempts() => _instance;
  TotalFitbAttempts._internal();

  @override
  String get name => "total_fitb_attempts";
  ...
  @override
  String get description => "Total number of Fill-in-the-blank attempts made across all time.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      // If we have a new FITB answer, increment the total
      ...
      QuizzerLogger.logError('Failed to recalculate total_fitb_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for total_fitb_attempts: $e');
      rethrow;
    }
  }
}