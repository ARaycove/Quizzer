...
class TodayTfTotalAttempts extends StatField{
  static final TodayTfTotalAttempts _instance = TodayTfTotalAttempts._internal();
  factory TodayTfTotalAttempts() => _instance;
  TodayTfTotalAttempts._internal();

  @override
  String get name => "today_tf_total_attempts";
  ...
  @override
  String get description => "Total number of True/False attempts made today.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      ...
      // If we have a new answer for a True/False question, increment
      if (isCorrect != null && questionType == 'true_false') {
        _cachedValue += 1;
      }
      
      return _cachedValue;
    } catch (e) {
      QuizzerLogger.logError('Failed to recalculate today_tf_total_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for today_tf_total_attempts: $e');
      rethrow;
    }
  }
}