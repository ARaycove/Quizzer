...
class TotalSoIncorrrectAttempts extends StatField{
  static final TotalSoIncorrrectAttempts _instance = TotalSoIncorrrectAttempts._internal();
  factory TotalSoIncorrrectAttempts() => _instance;
  TotalSoIncorrrectAttempts._internal();

  @override
  String get name => "total_so_incorrrect_attempts";
  ...
  @override
  String get description => "Total number of incorrect Sort Order attempts made across all time.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      // If we have a new incorrect Sort Order answer, increment the total
      if (isCorrect != null && !isCorrect && questionType == 'sort_order') {
        _cachedValue += 1;
      }
      // If isCorrect is null (just recalculating), keep current value
      return _cachedValue;
    } catch (e) {
      QuizzerLogger.logError('Failed to recalculate total_so_incorrrect_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for total_so_incorrrect_attempts: $e');
      rethrow;
    }
  }
}