...
class TotalSataCorrectAttempts extends StatField{
  static final TotalSataCorrectAttempts _instance = TotalSataCorrectAttempts._internal();
  factory TotalSataCorrectAttempts() => _instance;
  TotalSataCorrectAttempts._internal();

  @override
  String get name => "total_sata_correct_attempts";
  ...
  @override
  String get description => "Total number of correct Select All That Apply attempts made across all time.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      // If we have a new correct SATA answer, increment the total
      ...
      QuizzerLogger.logError('Failed to recalculate total_sata_correct_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for total_sata_correct_attempts: $e');
      rethrow;
    }
  }
}