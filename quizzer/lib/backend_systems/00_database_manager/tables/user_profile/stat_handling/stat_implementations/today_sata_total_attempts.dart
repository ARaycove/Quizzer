...
class TodaySataTotalAttempts extends StatField{
  static final TodaySataTotalAttempts _instance = TodaySataTotalAttempts._internal();
  factory TodaySataTotalAttempts() => _instance;
  TodaySataTotalAttempts._internal();

  @override
  String get name => "today_sata_total_attempts";
  ...
  @override
  String get description => "Total number of Select All That Apply attempts made today.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      ...
      // If we have a new answer for a SATA question, increment
      if (isCorrect != null && questionType == 'select_all_that_apply') {
        _cachedValue += 1;
      }
      
      return _cachedValue;
    } catch (e) {
      QuizzerLogger.logError('Failed to recalculate today_sata_total_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for today_sata_total_attempts: $e');
      rethrow;
    }
  }
}