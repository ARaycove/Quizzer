...
class TotalFitbIncorrrectAttempts extends StatField{
  static final TotalFitbIncorrrectAttempts _instance = TotalFitbIncorrrectAttempts._internal();
  factory TotalFitbIncorrrectAttempts() => _instance;
  TotalFitbIncorrrectAttempts._internal();

  @override
  String get name => "total_fitb_incorrrect_attempts";
  ...
  @override
  String get description => "Total number of incorrect Fill-in-the-blank attempts made across all time.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      // If we have a new incorrect FITB answer, increment the total
      if (isCorrect != null && !isCorrect && questionType == 'fill_in_the_blank') {
        _cachedValue += 1;
      }
      // If isCorrect is null (just recalculating), keep current value
      return _cachedValue;
    } catch (e) {
      QuizzerLogger.logError('Failed to recalculate total_fitb_incorrrect_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for total_fitb_incorrrect_attempts: $e');
      rethrow;
    }
  }
}