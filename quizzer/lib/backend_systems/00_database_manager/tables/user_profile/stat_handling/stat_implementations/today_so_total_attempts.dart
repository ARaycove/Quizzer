...
class TodaySoTotalAttempts extends StatField{
  static final TodaySoTotalAttempts _instance = TodaySoTotalAttempts._internal();
  factory TodaySoTotalAttempts() => _instance;
  TodaySoTotalAttempts._internal();

  @override
  String get name => "today_so_total_attempts";
  ...
  @override
  String get description => "Total number of Sort Order attempts made today.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      ...
      // If we have a new answer for a Sort Order question, increment
      ...
      QuizzerLogger.logError('Failed to recalculate today_so_total_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for today_so_total_attempts: $e');
      rethrow;
    }
  }
}