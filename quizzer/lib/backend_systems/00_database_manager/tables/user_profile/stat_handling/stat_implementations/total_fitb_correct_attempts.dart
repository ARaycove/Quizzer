...
class TotalFitbCorrectAttempts extends StatField{
  static final TotalFitbCorrectAttempts _instance = TotalFitbCorrectAttempts._internal();
  factory TotalFitbCorrectAttempts() => _instance;
  TotalFitbCorrectAttempts._internal();

  @override
  String get name => "total_fitb_correct_attempts";
  ...
  @override
  String get description => "Total number of correct Fill-in-the-blank attempts made across all time.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      // If we have a new correct FITB answer, increment the total
      ...
      QuizzerLogger.logError('Failed to recalculate total_fitb_correct_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for total_fitb_correct_attempts: $e');
      rethrow;
    }
  }
}