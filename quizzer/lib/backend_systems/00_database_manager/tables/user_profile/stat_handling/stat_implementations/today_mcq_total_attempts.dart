...
class TodayMcqTotalAttempts extends StatField{
  static final TodayMcqTotalAttempts _instance = TodayMcqTotalAttempts._internal();
  factory TodayMcqTotalAttempts() => _instance;
  TodayMcqTotalAttempts._internal();

  @override
  String get name => "today_mcq_total_attempts";
  ...
  @override
  String get description => "Total number of Multiple Choice attempts made today.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      ...
      // If we have a new answer for an MCQ question, increment
      if (isCorrect != null && questionType == 'multiple_choice') {
        _cachedValue += 1;
      }
      
      return _cachedValue;
    } catch (e) {
      QuizzerLogger.logError('Failed to recalculate today_mcq_total_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for today_mcq_total_attempts: $e');
      rethrow;
    }
  }
}