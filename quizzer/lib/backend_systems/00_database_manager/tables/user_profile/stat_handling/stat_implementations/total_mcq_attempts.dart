...
class TotalMcqAttempts extends StatField{
  static final TotalMcqAttempts _instance = TotalMcqAttempts._internal();
  factory TotalMcqAttempts() => _instance;
  TotalMcqAttempts._internal();

  @override
  String get name => "total_mcq_attempts";
  ...
  @override
  String get description => "Total number of Multiple Choice attempts made across all time.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      // If we have a new MCQ answer, increment the total
      ...
      QuizzerLogger.logError('Failed to recalculate total_mcq_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for total_mcq_attempts: $e');
      rethrow;
    }
  }
}