...
class TotalTfAttempts extends StatField{
  static final TotalTfAttempts _instance = TotalTfAttempts._internal();
  factory TotalTfAttempts() => _instance;
  TotalTfAttempts._internal();

  @override
  String get name => "total_tf_attempts";
  ...
  @override
  String get description => "Total number of True/False attempts made across all time.";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      // If we have a new True/False answer, increment the total
      ...
      QuizzerLogger.logError('Failed to recalculate total_tf_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for total_tf_attempts: $e');
      rethrow;
    }
  }
}