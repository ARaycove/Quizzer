...
class TotalIncorrectAttempts extends StatField{
  static final TotalIncorrectAttempts _instance = TotalIncorrectAttempts._internal();
  factory TotalIncorrectAttempts() => _instance;
  TotalIncorrectAttempts._internal();

  @override
  String get name => "total_incorrect_attempts";
  ...
  @override
  String get description => "Total number of incorrect attempts made across all time (all question types).";
  ...
  Future<int> recalculateStat({Transaction? txn, bool? isCorrect, double? reactionTime, String? questionId, String? questionType}) async {
    try {
      // If we have a new incorrect answer, increment the total
      ...
      QuizzerLogger.logError('Failed to recalculate total_incorrect_attempts: $e');
      rethrow;
    }
  }
  ...
  Future<int> calculateCarryForwardValue({Transaction? txn, Map<String, dynamic>? previousRecord, Map<String, dynamic>? currentIncompleteRecord}) async {
    try {
      ...
      QuizzerLogger.logError('Failed to calculate carry-forward for total_incorrect_attempts: $e');
      rethrow;
    }
  }
}