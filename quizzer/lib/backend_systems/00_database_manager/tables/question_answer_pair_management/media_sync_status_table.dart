...
Future<bool> _checkFileExistsLocally(String fileNameWithExtension) async {
  final String localPath = path.join(await getQuizzerMediaPath(), fileNameWithExtension);
...
Future<void> insertMediaSyncStatus({
  ...
  try {
    ...
    QuizzerLogger.logError('Error inserting media sync status - $e');
    rethrow;
  } finally {
...
Future<int> updateMediaSyncStatus({
  ...
  try {
    ...
    QuizzerLogger.logError('Error updating media sync status - $e');
    rethrow;
  } finally {
    getDatabaseMonitor().releaseDatabaseAccess();
  }
...
Future<Map<String, dynamic>?> getMediaSyncStatus(String fileName) async {
  try {
    final db = await getDatabaseMonitor().requestDatabaseAccess();
    if (db == null) {
      throw Exception('Failed to acquire database access');
    ...
    QuizzerLogger.logError('Error getting media sync status - $e');
    rethrow;
  } finally {
    getDatabaseMonitor().releaseDatabaseAccess();
  }
...
Future<List<Map<String, dynamic>>> getExistingLocallyNotExternally() async {
  try {
    final db = await getDatabaseMonitor().requestDatabaseAccess();
    if (db == null) {
      throw Exception('Failed to acquire database access');
    ...
    QuizzerLogger.logError('Error getting existing locally not externally records - $e');
    rethrow;
  } finally {
    getDatabaseMonitor().releaseDatabaseAccess();
  }
...
Future<List<Map<String, dynamic>>> getExistingExternallyNotLocally() async {
  try {
    final db = await getDatabaseMonitor().requestDatabaseAccess();
    if (db == null) {
      throw Exception('Failed to acquire database access');
    ...
    QuizzerLogger.logError('Error getting existing externally not locally records - $e');
    rethrow;
  } finally {
    getDatabaseMonitor().releaseDatabaseAccess();
  }
