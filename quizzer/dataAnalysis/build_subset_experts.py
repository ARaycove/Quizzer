...
from neural_net.prediction_net.model_sub   import update_submodel_record, report_sub_model_results

def build_subset_experts():
    # Data setup
    ...
    while True:
        ...
        # Out of the top_k subsets, find which one we should attempt next
        # FIXME We will update the find best subset to include some exploration.
        # While top_n and top_k ensure compute time only goes to the most promising results we should also scan those that are not in the top_k results, just in case some of those lucked out and got a bad hyperparameter set. . .
        ...
        # Extract feature list from the record, so we can pass it into the get_candidate_sub_records function
        s = subset_to_try['feature_set'].split(',')

        # Collect complete records based on the subset, extracting complete records from our train set
        ...
        trained_sub_model = train_sub_model(
            ...
            X_test              = X_test,
            y_test              = y_test,
            n_num_grid_search   = 1, #Do only 1 model grid search per iteration
            param_grid          = {
        ...
        update_submodel_record(trained_sub_model, subset_to_try) # specifically adds the model weights

        report_sub_model_results() # Call without sub_model results to refresh csv and histogram plot

if __name__ == "__main__":
    build_subset_experts()